...
func physics_process(delta: float) -> void:
	if goblin.wall_detected() or goblin.ledge_detected():
		goblin.set_direction(goblin.direction * -1)

	goblin.move(goblin.acceleration, goblin.direction, goblin.move_speed, delta)
